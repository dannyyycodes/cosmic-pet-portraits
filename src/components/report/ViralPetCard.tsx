...
const occasionThemes: Record<OccasionMode, {
  ...
  memorial: {
    ...
    badge: '🕊️ IN REMEMBRANCE 🕊️',
    tagline: (name) => `${name}, still carried`,
    viralCta: 'Share if their memory is still teaching you something',
    hashtags: ['#InRemembrance', '#StillCarried', '#PetMemorial', '#SoulReading'],
    viralPhrases: [
      'Some souls rewrite the people they loved',
      'The bond did not end. It changed shape',
      'A keeper, not a goodbye',
      'Still here in the habits they built in me',
      'The door I still check behind',
    ],
...
const getViralVibes = (archetype: string, stats: CardStats, petName: string, occasionMode: OccasionMode) => {
  ...
  if (occasionMode === 'memorial') {
    return {
      ...
      superpower: 'Still shaping the way you move through the world',
      secretTalent: 'Living in every habit they taught you',
      loveLanguage: 'A bond that changed shape, not ended',
      chaosLevel: 'Quietly Present',
    };
  }
