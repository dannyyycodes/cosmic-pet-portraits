...
export function ReportScrollProgress({ chapters }: ReportScrollProgressProps) {
  ...
  return (
    ...
    >
      {/* The rail */}
      <div className="relative w-px h-[60vh] bg-[#8b7bd8]/15 rounded-full overflow-hidden">
        <motion.div
          className="absolute inset-0 origin-bottom bg-[#8b7bd8]"
          style={{ scaleY }}
        />
      </div>
          ...
          >
            <span
              className={
                ...
                (i === active
                  ? 'w-2.5 h-2.5 bg-[#8b7bd8] shadow-[0_0_12px_rgba(139,123,216,0.6)]'
      ...
      <div className="mt-3 text-[0.6rem] tracking-[0.2em] uppercase text-[#8b7bd8] font-sans pointer-events-none">
        {String(active + 1).padStart(2, '0')}
        <span className="text-[#8b7bd8]/40"> / {String(chapters.length).padStart(2, '0')}</span>
      </div>
    </div>
  );
}
