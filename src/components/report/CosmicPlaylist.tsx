...
export function CosmicPlaylist({ petName, report }: CosmicPlaylistProps) {
  ...
  return (
    <motion.div
      ...
      variants={s.variants}
      className="mx-4 my-2.5 p-5 bg-[#1a1430] rounded-[14px] border border-[rgba(167,139,250,0.22)] max-w-[520px] sm:mx-auto"
    >
      ...
      {playlist.map((item, i) => (
        <div
          key={i}
          className="flex items-center gap-2.5 py-2 border-b border-[rgba(167,139,250,0.22)] last:border-b-0"
        >
          <span className="text-[0.7rem] font-bold text-[#a29ab8] w-5">
          ...
          >
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[0.82rem] font-semibold text-[#f2eeff] group-hover:underline decoration-[#1DB954]/40 underline-offset-2">
                {item.title}
              </span>
            </div>
            <div className="text-[0.7rem] text-[#a29ab8] ml-5">{item.artist}</div>
          </a>
          <span className="text-[0.62rem] text-[#a78bfa] font-semibold uppercase tracking-[0.5px]">
            {item.vibe}
          </span>
        </div>
      ))}
