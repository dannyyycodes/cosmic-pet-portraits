...
export function ReportMethodology({ petName }: ReportMethodologyProps) {
  ...
  return (
    <>
      ...
      >
        <div className="text-[0.55rem] font-bold tracking-[3px] uppercase text-[#8b7bd8]">
          Before Your Reading
        </div>
        <h2 className="text-[1.6rem] text-[#2a2440] leading-tight mt-2" style={{ fontFamily: 'DM Serif Display, serif' }}>
        ...
        <button
          ...
          style={{
            background: 'linear-gradient(135deg, rgba(139,123,216,0.12), rgba(139,123,216,0.06))',
        ...
        {isExpanded && (
          ...
          >
            <div className="relative flex flex-col max-w-[520px] mx-auto px-6">
              {/* Connecting line */}
              <div className="absolute left-[46px] top-7 bottom-7 w-[1.5px] bg-gradient-to-b from-[#8b7bd8] to-[#e2dbf3]" />
              ...
              {steps.map((step) => (
                ...
                >
                  <div className="w-11 h-11 rounded-full flex-shrink-0 bg-white border-[1.5px] border-[#8b7bd8] flex items-center justify-center text-[0.75rem] font-bold text-[#8b7bd8] relative z-[1]">
                    {step.num}
                  </div>
                  <div>
                    <h4 className="font-dm-serif text-[0.95rem] text-[#2a2440] mb-0.5">
                      {step.title}
                    </h4>
                    <p className="text-[0.78rem] text-[#928aa8] leading-[1.55]">
                      {step.desc(petName)}
                    </p>
                  </div>
                </div>
