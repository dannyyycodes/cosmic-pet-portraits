...
try {
  ...
  const occasionMode: "discover" | "new" | "birthday" | "memorial" | "gift" =
  ...
  const modeContext: Record<string, string> = {
    discover: "This is a discovery reading - help the owner truly understand their pet for the first time. Tone: curious, exciting, revelatory. Use PRESENT TENSE throughout (is, loves, brings, has). The owner has had this pet a while — this reading is the ANSWER to patterns they've watched, the naming of quirks they've noticed, the permission to trust what they already half-knew. Lead with 'you might have noticed' and 'that makes sense now' language. This is recognition, not introduction.",
    new: `This is a NEW-PET ARRIVAL reading - ${name} has just entered the owner's life. This is NOT a discovery reading — the owner does not yet have months of patterns to recognise. The voice must be ARRIVAL + WELCOME + FORECAST, not revelation. Tone: warm welcome, curiosity about who ${pronouns.subject} will become, tender excitement. Use PRESENT TENSE throughout (is, has, brings, loves). Speak as someone handing the owner a map of a soul they've just been entrusted with. Reframe rules: where 'discover' says "you've probably noticed ${pronouns.subject} does X", 'new' says "watch for ${pronouns.subject} to do X — it's a ${sunSign} Sun signature". Where 'discover' says "that explains why", 'new' says "here's what to expect". Include ONE explicit "the first few weeks" section-voice cue: what bonding gesture ${pronouns.subject} will likely offer first, what ${pronouns.subject} will need from the owner in the settling-in window. Never write sentences that assume the owner has already witnessed behaviour. Every observation is framed as "this chart points to..." not "you've seen...".`,
    birthday: `This is a birthday celebration reading - honor how their pet has grown and the joy they bring. Tone: celebratory, warm, grateful. Use PRESENT TENSE throughout (is, loves, brings, has). Required beats throughout the report: (1) A gratitude-forward prologue that names THIS year specifically — "another year of ${name}" framing, not generic celebration. (2) In the main sections, weave in at least 2 "the year behind" moments where the reading reflects on growth the owner has witnessed. (3) In the practical/predictive sections, shift to "the year ahead" language — what this next solar return invites. (4) Epilogue includes a direct birthday blessing: a line the owner could read aloud to ${name} on the day.`,
  ...
  const modeEmotionalGuidance: Record<string, string> = {
    ...
    new: `Use arrival-voice warmth. 'Watch for ${pronouns.subject} to...' 'You can expect ${pronouns.object} to...' 'In the first few weeks...' 'As ${pronouns.subject} settles in...' Include an explicit welcome moment in the prologue — the reader just brought ${pronouns.object} home, honour that. Forecast 2–3 bonding gestures the chart points to (the way ${pronouns.subject} will likely ask for comfort, the way ${pronouns.subject} will first show trust, the way ${pronouns.subject} will claim ${pronouns.possessive} favourite spot). Present tense but forward-leaning: '${name} IS ${sunSign}, which means ${pronouns.subject} will naturally...' Avoid "you've probably noticed" — the owner hasn't noticed anything yet.`,
    birthday: `Celebrate ${pronouns.possessive} journey. Use the solar-return framing: this birthday is a NEW chart year and the Sun returns to its natal degree. Open with gratitude for the year behind ("another year of ${name}'s ${sunSign} light..."), then use the mid sections to reflect on growth the owner has watched, then shift forward-facing in the practical/predictive sections ("this solar return invites..."). Include a direct birthday blessing in the epilogue — a line the owner can read aloud to ${name} on the day. Present tense throughout: '${name} IS... ${name} BRINGS...' Include gratitude-forward language ('the cosmic gifts ${pronouns.subject} brings', 'a year of ${pronouns.possessive}...', 'everything ${pronouns.subject} has become').`,
  ...
  for (let pass = 0; pass < 2; pass++) {
    ...
    try {
      const verification = await verifyReport(
        ...
        {
          gender,
          occasionMode: (occasionMode === "memorial" || occasionMode === "birthday" || occasionMode === "gift" || occasionMode === "new") ? occasionMode : "discover",
          petName: name,
          language,
          validSigns,
          chartDegrees,
